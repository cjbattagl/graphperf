...
A rule-of-thumb would be that we should consolidate when the following cost inequality is satisfied: \begin{align} flops(A + A_u) + \sum_{i=0}^N flops( A'x_i) \leq \sum_{i=0}^N [flops(Ax_i) + flops(A_ux_i)]\end{align}



\begin{align*}
\text{operation} && \text{flops} && \text{bytes} \\
A+A_u && 2(|E(A)|+|E(A_u)|) && (12ánnz +4á(n+1) +8á(n \cdots nnz) + 8án) \\
A'x_i && 2(|E(A)|+|E(A_u)|) \\
Ax_ i && 2 |E(A)| \\
A_ux_i && 2|E(A_u)|
\end{align*}
